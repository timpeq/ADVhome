...
2. **T-04 · Screenshots or photos for the README and forum post** `tim` `device`
...
3. **T-07 · Publish the blog posts** `tim`
...
4. **T-08 · Try light brightness on a dimmable light** `device`
    Built in `757c6f8` but never run against a real dimmable light, because the
...
5. **T-09 · Clear "Speaking" when the sound stops, not the stream** `agent` `device`
...
6. **T-10 · Surface `P` and `S` in the media player's hint, or drop them** `agent`
    Play/pause and stop work, but only the Help page mentions them.

7. **T-11 · Check the detail window layout for every domain** `agent` `device`
    With short and long state strings, and with `unavailable`. T-29's rendering
    fixture could do this without the device.

8. **T-12 · Make ESC an escalating back button** `agent`
...
12. **T-16 · Reconfigure without rebooting** `agent`
...
27. **T-32 · Measure the reconnect after deep sleep** `device`
...
28. **T-33 · Check what `WIFI_PS_MAX_MODEM` costs** `device`
...
29. **T-34 · Put the IMU in low-power mode between polls** `agent` `device`
    It runs continuously, even with the screen off.

30. **T-35 · Put the TCA8418 in low-power mode while asleep** `agent` `device`
    If the keyboard controller supports it.

31. **T-36 · Barge-in over `wss`** `agent` `device`
...
32. **T-37 · Timed wake, if it is ever wanted** `tim`
    Refresh state periodically without user input (`M5.Power.timerSleep`).
    Decide whether it is wanted before building it.

33. **T-38 · Skip the USB wait on deep-sleep wakes that pass through M5Launcher** `agent` `device`
